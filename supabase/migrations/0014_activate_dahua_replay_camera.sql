...
update items set
  ...
  -- NOT the default. The Uniview keeps it, so a venue that has not chosen is
  -- sized exactly as it was before this migration ran.
  is_default = false,
...
-- Every EXISTING venue is pinned to the Uniview in the same transaction that
-- creates the alternative. A choice row exists if and only if the role has
-- ever had more than one active item — otherwise whether a venue is protected
-- from a later default flip would depend on whether someone happened to save
-- it, which is not a rule anyone can reason about.
--
-- Both preconditions are asserted rather than assumed. Written as a plain
-- CROSS JOIN LATERAL, a missing default yields zero rows for EVERY venue and
-- the migration "succeeds" having pinned nobody — and criterion 4 rests
-- entirely on this insert. The name predicate above is likewise the only
-- handle this migration has on the Dahua row.
do $$
declare
  v_default uuid;
begin
