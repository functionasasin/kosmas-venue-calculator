...
describe('readTier', () => {
  ...
  // an older deck, spreadsheet field, or quote, read it as Basic+."
  it('reads a legacy "basic" row as basic_plus, because the tier column is plain text and rows predating the retirement still hold it', () => {
  ...
  it('leaves every live tier untouched, so the coercion cannot mask a real value', () => {
    for (const t of ['basic_plus', 'pro', 'pro_plus', 'autonomous', 'autonomous_plus'] as const) {
